...
% AR parametrelelerinin Yule-Walker Metodu ile hesaplanmasý(ARYULE)
...
for i=1:size(ecg_nsr,2)
    ar_nsr(:,i)=aryule(ecg_nsr(:,i),10); % ECG sinyali için 4.dereceden AR parametleri-Normal sinüs ritmi
end
...
for i=1:size(ecg_afib,2)
    ar_afib(:,i)=aryule(ecg_afib(:,i),10); % ECG sinyali için 4.dereceden AR parametleri- Sað dal bloku
end
...
for i=1:size(ecg_nsr_test,2)
    ar_nsr_test(:,i)=aryule(ecg_nsr_test(:,i),10); % ECG sinyali için 4.dereceden AR parametleri-Normal sinüs ritmi
end
for i=1:size(ecg_afib_test,2)
    ar_afib_test(:,i)=aryule(ecg_afib_test(:,i),10); % ECG sinyali için 4.dereceden AR parametleri- Sað dal bloku
end
