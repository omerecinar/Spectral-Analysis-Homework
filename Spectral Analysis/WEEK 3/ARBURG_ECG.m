...
% AR parametrelelerinin Burg Metodu ile hesaplanmasý(ARBURG)
...
for i=1:size(ecg_nsr,2)
    ar_nsr(:,i)=arburg(ecg_nsr(:,i),4); % ECG sinyali için 4.dereceden AR parametleri-Normal sinüs ritmi
end
...
for i=1:size(ecg_sa,2)
    ar_sa(:,i)=arburg(ecg_sa(:,i),4); % ECG sinyali için 4.dereceden AR parametleri- Sað dal bloku
end
Hs=spectrum.burg(4);                   % 4th order AR model
subplot(2,1,2);psd(Hs,ecg_sa(:,i),'NFFT',512);title('AFÝB için PSD')

% TEST verileri için AR parametrelerinin hesaplanmasý
for i=1:size(ecg_nsr_test,2)
    ar_nsr_test(:,i)=arburg(ecg_nsr_test(:,i),4); % ECG sinyali için 4.dereceden AR parametleri-Normal sinüs ritmi
end
for i=1:size(ecg_sa_test,2)
    ar_sa_test(:,i)=arburg(ecg_sa_test(:,i),4); % ECG sinyali için 4.dereceden AR parametleri- Sað dal bloku
end

%============================================

% Bu AR parametreleri kullanýlarak sinyalin sýnýflandýrýlmasý
train_input=[ar_nsr ar_sa];
train_hedef=zeros(1,35);
train_hedef(1,1:20)=0;
train_hedef(1,21:35)=1;

test_input=[ar_nsr_test ar_sa_test];
